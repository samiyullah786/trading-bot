...
class ToolRuntime:
    ...
    def __init__(
        ...
        verifier: Callable[[ToolRequest, ToolResult], Verification] | None = None,
        max_attempts: int = 2,
        max_observation: int = 12000,
    ) -> None:
    ...
    def execute(self, request: ToolRequest) -> ToolExecution:
        ...
        for attempt in range(1, self.max_attempts + 1):
            try:
                result = self.registry.execute(request.intent if request.intent in self.registry.names() else request.payload.get("tool", request.intent), request)
