...
class AutonomousLoop:
    """Outcome loop with explicit state semantics and evidence-gated completion."""
    def __init__(self, kernel: OutcomeKernel, strategist: Strategist, executor: Callable[[ProposedAction], tuple[bool, str, list[str]]] | None = None):
        self.kernel = kernel
    ...
    def cycle(self) -> dict:
        ...
        if not proposals:
            return {"state": "BLOCKED", "reason": "no action proposed", "gaps": gaps, "report": self.kernel.report()}
        ...
        if self.executor is None:
            return {"state": "READY", "action": action.id, "description": action.description, "gaps": gaps, "report": self.kernel.report()}
        ...
        if success:
            ...
            return {"state": "PROGRESS", "action": action.id, "complete": report["complete"], "report": report}
