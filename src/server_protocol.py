...
class ServerRequest:
    ...
    def to_bytes(self) -> bytes:
        ...
        return json.dumps(
            {"v": self.protocol_version, "id": self.request_id, "op": self.operation, "payload": self.payload},
            separators=(",", ":"),
            sort_keys=True,
        ).encode("utf-8")
...
class ServerResponse:
    ...
    def to_bytes(self) -> bytes:
        body = {"v": self.protocol_version, "id": self.request_id, "ok": self.ok, "result": self.result}
    ...
    def from_bytes(cls, raw: bytes) -> "ServerResponse":
        ...
        return cls(str(value.get("id", "")), bool(value.get("ok")), result, value.get("error"), str(value["v"]))
